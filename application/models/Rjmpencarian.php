...
where daftar_ulang_irj.id_poli='$id_poli' and daftar_ulang_irj.no_medrec=data_pasien.no_medrec  
...
//SELECT * FROM daftar_ulang_irj, data_pasien where daftar_ulang_irj.id_poli='$id_poli' and daftar_ulang_irj.no_medrec=data_pasien.no_medrec  and left(daftar_ulang_irj.tgl_kunjungan,10) = left(now(),10) and daftar_ulang_irj.status='0'
		...
		function get_pasien_daftar_by_noPPI($ID_SENSUS,$NO_REGISTER){
			return $this->db->query("SELECT * FROM PASIEN_PPI, data_pasien where PASIEN_PPI.ID_SENSUS='$ID_SENSUS' and PASIEN_PPI.NO_REGISTER=data_pasien.NO_REGISTER  and data_pasien.NO_REGISTER='$NO_REGISTER'");
		}
